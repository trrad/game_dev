...
import { SceneEvents } from "../scene/SceneGraphEventSystem";
...
export interface RadiusFilter {
    ...
    customFilter?: (node: NodeComponent, radius: RadiusComponent) => boolean;
}
...
export class RadiusComponent extends Component<RadiusComponentData> {
    ...
    /**
     * Get world position from attached GameObject's NodeComponent
     */
    private getWorldPosition(): Vector3 | null {
        if (!this._gameObject) return null;
        
        const sceneNode = this._gameObject.getComponent<NodeComponent>('sceneNode');
     ...
     */
    getNodesInRadius(filter?: (node: NodeComponent) => boolean): NodeComponent[] {
    ...
    emitToRadius(eventType: string, payload?: any, filter?: RadiusFilter): void {
        ...
        const nodeFilter = (node: NodeComponent): boolean => {
