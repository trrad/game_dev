...
import { InventoryComponent } from '../../../../engine/components/InventoryComponent';
...
import { TrainCarVoxel } from './TrainCarVoxels/TrainCarVoxel';
 ...
 */
export class TrainCar extends GameNodeObject {
    ...
    constructor(config: TrainCarConfig, eventStack?: any, scene?: Scene, parentSceneNode?: NodeComponent) {
        super('trainCar', eventStack, scene, parentSceneNode);
        this._config = config;
    ...
    getVoxelsInRadius(center: { x: number; y: number; z: number }, radius: number): TrainCarVoxel[] {
        ...
        return nodesInRadius
            .map(node => node.gameObject as unknown as TrainCarVoxel)
